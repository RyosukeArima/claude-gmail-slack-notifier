...
### Step 8: ワンクリック下書き機能（オプション）

ユーザーに聞く: 「Slackの通知にワンクリックで返信下書きを作成するボタンを追加できます。設定しますか？（スキップ可能）」

スキップする場合 → Step 9へ進む。

設定する場合、`/mail-setup` の Step 6 と同じ手順で案内する。

### Step 9: 初回チェック + Cron登録
...
5. 結果がある場合:
   ...
   e. `config.json` に `gasWebAppUrl` が設定されている場合、各返信案をGAS Web Appに保存する:
      ...
      - 返信案データをbase64エンコードしてGETリクエストで送信:
      ```
      DATA=$(echo -n '{"messageId":"<id>","subject":"<件名>","replyBody":"<返信案>"}' | base64)
      curl -sL "<gasWebAppUrl>?action=save&token=<gasAuthToken>&id=<UUID>&data=${DATA}" -o /dev/null
      ```
      - 生成したUUIDを各メールに紐づける。
...
- `gasWebAppUrl` が未設定の場合は、従来通り `accessory` 形式で「Gmailで開く」ボタンのみ表示する
