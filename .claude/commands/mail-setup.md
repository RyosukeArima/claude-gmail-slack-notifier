...
4. 「左メニューの歯車アイコン（プロジェクトの設定）を開いて、一番下の『スクリプト プロパティ』に以下を追加してください。」
   → ランダムな認証トークンを生成して表示する:
   - プロパティ: `AUTH_TOKEN`
   - 値: `（生成したトークン）`
5. 「右上の『デプロイ』→『新しいデプロイ』を選んでください。」
6. 「歯車アイコン →『ウェブアプリ』を選択。『次のユーザーとして実行』を『自分』、『アクセスできるユーザー』を『全員』にして『デプロイ』を押してください。」
7. 「初回はGmailへのアクセス許可が求められます。『権限を確認』→ Googleアカウントを選択 →『詳細』→『安全でないページに移動』→『許可』を押してください。」
8. 「表示されたウェブアプリのURL（https://script.google.com/macros/s/.../exec の形式）をコピーして、ここに貼り付けてください。」

URLを受け取ったら `config.json` に保存する:
```json
{
...
テスト: GASにテストPOSTを送信して動作確認する:
```bash
curl -s -X POST -H 'Content-Type: application/json' \
  -d '{"token":"<トークン>","items":[{"messageId":"test","subject":"テスト","replyBody":"テスト返信"}]}' \
  "<URL>"
```
成功レスポンス（`results` 配列が返る）を確認する。
