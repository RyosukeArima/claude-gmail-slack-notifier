...
3. 成功を確認したら、シェルのrcファイルに追記し、現在のセッションにも反映する。
   `echo $SHELL` でシェルを判定し、適切なrcファイルを使う:
   - `/bin/zsh` → `~/.zshrc`
   - `/bin/bash` → `~/.bashrc`
   - その他 → ユーザーに確認する
   ```bash
   echo 'export SLACK_WEBHOOK_URL="受け取ったURL"' >> ~/.<shell>rc
...
2. 受け取ったIDをStep 4と同じrcファイルに追記し、現在のセッションにも反映する:
   ```bash
   echo 'export SLACK_USER_ID="受け取ったID"' >> ~/.<shell>rc
